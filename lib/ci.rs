...
impl ContinuousIntegration {
    ...
    pub fn run(&mut self) -> Result<()> {
        ...
        let report = self.save_html_report(&log_path, &header)?;

        if self.should_fail() {
            if let Some(email) = self.config.email() {
                EmailReport::new(email, report, &header, self.config.host()).send()?;
            }
    ...
    fn save_html_report(&self, log_path: &Path, header: &str) -> Result<File> {
